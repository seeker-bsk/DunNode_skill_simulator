...
typedef struct Skill {
    ...
    float lv1_total_damage;       // 1레벨 총 데미지%
    float lvmax_total_damage;     // master_level 총 데미지%
...
float get_damage_at_level(Skill* s, int level) {
    if (s->master_level <= 1) return s->lv1_total_damage;
    float t = (float)(level - 1) / (float)(s->master_level - 1);
    return s->lv1_total_damage + (s->lvmax_total_damage - s->lv1_total_damage) * t;
}
...
**merge 처리 내용:**
1. `damageSources` 기반으로 레벨별 `lv1_total_damage`, `lvmax_total_damage` 계산 (히트수 반영)
2. `masterLevelOverride` 적용 또는 기본 공식(`api_max_level - 10`) 사용
  ...
  "skills": [
    {
      ...
      "sp_cost_per_level": 1,
      "level_mode": "sp",
      "lv1_total_damage": 6670.0,
      "lvmax_total_damage": 53360.0,
      "base_cooldown": 3.5,
      "cast_time": 0.0,
      "enhancement_type": 0,
      "bloom_type": 0,
