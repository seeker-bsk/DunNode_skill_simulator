...
각 스킬의 current_level 결정:
  LEVEL_MODE_AUTO_CHAR    → current_level = min(char_level, master_level)
                            // 달인의 계약 미적용 (기본기 숙련 115레벨 고정)
...
| ------ | --------------------------------------------- | ------------------------------------- |
| GET    | `/ping`                                       | 헬스체크                              |
| GET    | `/jobs`                                       | 직업 목록 + hasData 여부 반환         |
| GET    | `/tree/:dataKey`                              | 직업별 스킬 트리 데이터 반환          |
| POST   | `/api/simulate`                               | 시뮬레이션 실행                       |
| POST   | `/skills/rebuild/:dataKey`                    | 스킬 데이터 재빌드 (merge)            |
| GET    | `/api/character/servers`                      | 한국 서버 목록 반환 (정적)            |
| GET    | `/api/character/search?name={}&server={}`     | Neople API 캐릭터 검색                |
| GET    | `/api/character/:serverId/:characterId`       | 캐릭터 직업 정보 + 스킬 레벨 조회    |

---

## 캐릭터 검색 기능 (M9)

Neople 공식 API로 캐릭터를 조회하여, 해당 캐릭터의 직업 스킬 트리를 자동 로드하고 스킬 레벨을 적용한다.
스탯(쿨감 등)은 자동 적용하지 않는다 — 장비 파싱이 복잡하므로 수동 유지.

### 사용 API 엔드포인트

```
# 캐릭터 검색 (서버 + 이름)
GET https://api.neople.co.kr/df/servers/{serverId}/characters
    ?characterName={name}&wordType=match&apikey={key}

# 캐릭터 직업 정보
GET https://api.neople.co.kr/df/servers/{serverId}/characters/{characterId}
    ?apikey={key}

# 캐릭터 스킬 스타일 (실제 스킬 레벨)
GET https://api.neople.co.kr/df/servers/{serverId}/characters/{characterId}/skill/style
    ?apikey={key}
```
...
```
프론트 → GET /api/character/search?name=닉네임&server=cain
       ← [{ characterId, characterName, level, serverId, serverName, jobId, jobGrowId }]

사용자가 목록에서 캐릭터 선택

프론트 → GET /api/character/{serverId}/{characterId}
       ← {
           dataKey: "{jobId}_{jobGrowId}",    // jobs.json으로 매칭
           hasData: true/false,               // merged.json 존재 여부
           skillLevels: [{ skillId, level }]  // skill/style API에서 추출
         }

프론트: setSelectedJob(job where job.dataKey === dataKey)
        + skills 배열에서 skillId 일치하는 항목의 current_level 업데이트
```

### Node.js 구현 주의사항

- `NEOPLE_API_KEY`는 반드시 `process.env.NEOPLE_API_KEY`로만 사용. 코드에 하드코딩 금지.
- Neople API 응답 timeout은 8초 (외부 API이므로 simulator용 5초보다 여유 있게).
- 캐릭터의 `jobGrowId`가 `jobs.json`에 없으면 `hasData: false`로 응답.
- `skill/style` 응답의 `active` 배열에서 `skillId`와 `level`만 추출해 반환.
- `/api/character/servers`는 정적 배열 반환 — 외부 API 호출 없음.

### 프론트엔드 UI 위치

헤더(Header.jsx) 오른쪽 영역에 배치:
```
[서버 선택 ▼] [캐릭터명 입력____________] [검색]
                    ↓ 결과 드롭다운 (최대 10건)
```
검색 결과 선택 시: 직업 자동 전환 + 스킬 레벨 일괄 적용 + 토스트 "OO님의 스킬 트리를 불러왔습니다"
...
- [x] M5: Node.js 서버
- [x] M6: 프론트엔드 — 트리 시각화
- [x] M7: 프론트엔드 — 스펙 패널 + 결과 출력
- [x] M7.5: 프론트엔드 — 딜량 분석 패널 (차트 + 스킬 통계 + 아이콘 타임라인)
- [x] M8: 통합 테스트
- [x] M9: 캐릭터 검색 — Neople API로 캐릭터 조회 후 직업/스킬 레벨 자동 로드
- [x] M10: 스킬 툴팁 — hover 600ms 후 포털 툴팁 표시 + 개화/강화 수동 선택 UI
